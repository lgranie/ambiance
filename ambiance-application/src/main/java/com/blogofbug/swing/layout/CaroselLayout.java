...
import com.blogofbug.swing.components.RichComponent;
import com.blogofbug.swing.components.effects.AlphaEffect;
import com.blogofbug.swing.components.effects.BloomEffect;
...
import com.blogofbug.swing.components.effects.SlowPaintEffect;
...
public class CaroselLayout implements LayoutManager,ActionListener{
    ...
    private ContainerEffectManager effectManager;
     ...
     * @param componentEffects The layout imposes effects on the different components, these
     * are acquired from the ContainerEffectManager
     */
    public CaroselLayout(Container forContainer,ContainerEffectManager componentEffects){
//        animationTimer = new Timer(0,this);
        container = forContainer;
        effectManager = componentEffects;
    }
    ...
    public void removeLayoutComponent(Component comp) {
        CaroselPosition cp = getPosition(comp);
/*        if (cp!=null){
            cp.removeEffect();
        }*/
        components.remove(comp);
    ...
    protected CaroselPosition getPosition(Component comp){
        ...
        if (cpos==null){
            AlphaEffect frontMostEffect = null;
            if (comp instanceof RichComponent){
                frontMostEffect = new BloomEffect(ImageUtilities.renderComponentToImage((JComponent) comp),1.0f);
            }
            cpos = new CaroselPosition(comp);//,frontMostEffect);
    ...
    protected void recalculateCarosel(){
        ...
        try{
            ...
            if (animate){
                ...
                fireAnimationEvent(ANIMATION_STARTED);
//                animationTimer.start();
    ...
    protected Point calculateCenter(Insets insets, int width, int height, int widest){
        return new Point((insets.left+widest/2) + width/2, (insets.top + height/2) - height/8);
    }
    ...
    /**
     * May be overridden, returns, for a given angle (final rendered angle) the
     * brightness of any foreground effect.
     *
     * @param finalAngle The final angle of the component
     * @return A number between 0.0 and 1.0, although 0.5 is normally as bright as needed
     */
    protected float getFrontmostAlpha(float finalAngle){
        float effectAlpha = (float) Math.cos(finalAngle);
    ...
    public void layoutContainer(Container target) {
        ...
        while (i.hasNext()){
            ...
            double initialWidth = (double) neutralContentWidth;
...
//            if (position.frontMostEffect!=null){
                CarouselComponentEffect cce = (CarouselComponentEffect) effectManager.getEffectFor((JComponent) comp);
                float alpha = getFrontmostAlpha((float) finalAngle);
                cce.getTextEffect().setAlpha(alpha);
                if (cce.getHighlight() instanceof AlphaEffect){
                    ((AlphaEffect) cce.getHighlight()).setAlpha(alpha);                    
                }
//                position.frontMostEffect.setAlpha(alpha);
//            }
...
//            return false;
//        }
        if (!SwingBugUtilities.isTimerListening(this)){
            return false;
        }
...
//            animationTimer.stop();
            fireAnimationEvent(ANIMATION_COMPLETE);
            return;
        }
...
//        }
	...
	public Component getFrontMostComponent() { return container.getComponent(0); }
...
//        protected AlphaEffect  frontMostEffect;
        
        public CaroselPosition(Component component){   //, AlphaEffect theFrontMostEffect){
            angle = 0.0;
            scale = 0.0;
            z = 0.0;
            this.component = component;
            ComponentEffect highlightEffect = effectManager.getEffectFor((JComponent) component);
            //Cheating really, but it's a slow paint effect
            if (highlightEffect instanceof SlowPaintEffect){
                SlowPaintEffect bc = (SlowPaintEffect) highlightEffect;
                bc.removePaintLock();
                highlightEffect.initializeEffect();
            }
//            if (component instanceof RichComponent ){
//                ((RichComponent) component).addForegroundEffect(theFrontMostEffect);
//                frontMostEffect=theFrontMostEffect;
 //           }
        }
        ...
        public void setEffectAlpha(float alpha){
            if (frontMostEffect!=null){
                frontMostEffect.setAlpha(alpha);
            }
        }
        
        public void removeEffect(){
            if (frontMostEffect!=null){
                ((RichComponent) component).removeForegroundEffect(frontMostEffect);
            }
        }*/
	...
	{
            return getNextComponent(getFrontMostComponent());
	}
	...
	{
            return getPreviousComponent(getFrontMostComponent());
	}
	

	// AnimationListener
	public void addAnimationListener(AnimationListener l) { animationListeners.add(l); }
	public void removeAnimationListener(AnimationListener l) { animationListeners.remove(l); }
	private List<AnimationListener> animationListeners = new ArrayList<AnimationListener>();
	
    private void fireAnimationEvent(int type)
    {
    	for (AnimationListener lAnimationListener : animationListeners) 
    	{
    		if (type == ANIMATION_STARTED) lAnimationListener.animationStarted();
    		if (type == ANIMATION_COMPLETE) lAnimationListener.animationComplete();
    	}
    }
	final static public int ANIMATION_STARTED = 1;
	final static public int ANIMATION_COMPLETE = 2;
